...
void testReferenceGpuInferenceWhenConfigured() {
  ...
  const auto filename = points_bin.filename().string();
  const std::string replay_suffix = ".pointcloud.bin";
  const bool replay_input = filename.size() >= replay_suffix.size() &&
      filename.compare(filename.size() - replay_suffix.size(), replay_suffix.size(),
                       replay_suffix) == 0;
  if (replay_input) {
    fmcw::PointCloudReplayReader reader;
    fmcw::PointCloudSnapshot frame;
    std::string replay_error;
    expect(reader.open(points_bin, replay_error),
           "reference Waymo replay opens: " + replay_error);
    expect(reader.readNext(frame, replay_error) == fmcw::PointCloudReadResult::FrameReady,
           "reference Waymo replay provides its first frame: " + replay_error);
    input = fmcw::buildCenterPointInput(frame);
    expect(frame.feature_encoding == fmcw::PointCloudFeatureEncoding::CenterPointWaymo,
           "reference replay marks the Waymo model feature contract");
  } else {
    ...
    input.features.resize(static_cast<std::size_t>(byte_count) / sizeof(float));
    stream.seekg(0, std::ios::beg);
    stream.read(reinterpret_cast<char*>(input.features.data()),
    ...
    input.source_points = input.features.size() / 5U;
  }
