...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
    ...
    HWND hwnd = CreateWindow(
        wc.lpszClassName, _T("ATS GUI"),
        WS_OVERLAPPEDWINDOW, 100, 100, 1400, 920,
    ...
    auto stop_and_join = [&]() {
        ...
        if (boardHandle) AlazarAbortAsyncRead(boardHandle);
        if (acqThread.joinable()) acqThread.join();
    ...
    while (!done)
    {
        ...
        // --------------------------
        {
            ...
            // plot length
            int plen_max = g_plot_len_max.load(std::memory_order_relaxed);
            int plen = g_plot_len.load(std::memory_order_relaxed);
            if (plen_max > 0) {
                if (plen <= 0) plen = plen_max;
                ImGui::SameLine();
                ImGui::SetNextItemWidth(220);
                if (ImGui::SliderInt("##plen", &plen, 1, plen_max))
                    g_plot_len.store(plen, std::memory_order_relaxed);
                ImGui::SameLine();
                ImGui::Text("Plot len");
            }
        ...
        float min_log_h = 120.0f;
        float max_log_h = (avail.y > 200.0f) ? (avail.y * 0.6f) : 200.0f;
        if (log_h < min_log_h) log_h = min_log_h;
        if (log_h > max_log_h) log_h = max_log_h;

        float plot_h = avail.y - log_h - 8.0f;
        if (plot_h < 80.0f) plot_h = 80.0f;

        // ==========================
        // Plot panel
        // ==========================
        ImGui::BeginChild("PlotPanel", ImVec2(0, plot_h), true, ImGuiWindowFlags_NoMove);
        ...
        int K = g_fft_len.load(std::memory_order_relaxed);
        if (K > 0) {
        ...
        // panel height split: 4 plots
        float panel_h = ImGui::GetContentRegionAvail().y;
        float gap = 8.0f;
        float h_each = (panel_h - gap * 3.0f) * 0.25f;
        if (h_each < 80.0f) h_each = 80.0f;

        // ----------------------
        // 1) Time Domain
        // ----------------------
        if (!local_time.empty()) {
            if (ImPlot::BeginPlot("Time Domain", ImVec2(-1, h_each))) {

                if (autoFitY) {
                    ImPlot::SetupAxes("sample", "amp",
        ...
        // ----------------------
        if (!local_fft.empty()) {
            if (ImPlot::BeginPlot("FFT (r-th record, dB)", ImVec2(-1, h_each))) {

                ImPlot::SetupAxes("bin", "dB",
                    ImPlotAxisFlags_AutoFit,
                    ImPlotAxisFlags_AutoFit);
        ...
        if (!local_peak_idx.empty() && !batch_x.empty() &&
            ...
            if (ImPlot::BeginPlot("Peak Index vs Batch", ImVec2(-1, h_each))) {
                ImPlot::SetupAxes("batch", "peak idx",
                    ImPlotAxisFlags_AutoFit,
                    ImPlotAxisFlags_AutoFit);

                ImPlot::PlotLine("PeakIdx", batch_x.data(), local_peak_idx.data(), (int)local_peak_idx.size());
                ImPlot::EndPlot();
            }
        }
        else {
        ...
        if (!local_peak_val.empty() && !batch_x.empty() &&
            ...
            if (ImPlot::BeginPlot("Peak Value vs Batch", ImVec2(-1, h_each))) {
                ImPlot::SetupAxes("batch", "peak val (dB)",
                    ImPlotAxisFlags_AutoFit,
                    ImPlotAxisFlags_AutoFit);

                ImPlot::PlotLine("PeakVal", batch_x.data(), local_peak_val.data(), (int)local_peak_val.size());
                ImPlot::EndPlot();
            }
        }
        else {
        ...
        ImGui::EndChild(); // PlotPanel
        ImGui::Spacing();

        // ==========================
        // Log panel
        // ==========================
        ImGui::BeginChild("LogPanel", ImVec2(0, 0), true, ImGuiWindowFlags_NoMove);
        ...
        ImGui::SliderFloat("##logh", &log_h, 120.0f, 420.0f, "Log H=%.0f");
        ...
        if (log_auto_scroll) {
            ...
            if (y >= y_max - 2.0f) ImGui::SetScrollHereY(1.0f);
        }
