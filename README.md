...
**기본 계측 · FFT · Live 3D 표시 · 저장 · RAW 재생**

작업 브랜치: `codex/fmcw-base`

## 실행

이 폴더의 **[RUN.cmd](RUN.cmd)를 더블클릭**합니다.

실제 실행 파일: `build/package/Windows-Basic/FMCW_LiDAR.exe`

다른 PC로 실행본을 옮길 때는 **`Windows-Basic` 폴더 전체**를 복사합니다.
...
[FMCW_Basic.code-workspace](FMCW_Basic.code-workspace)를 열고, [작업 공간 관리·Windows 빌드 안내](docs/workspaces_ko.md)를 따릅니다.
이 제품의 preset은 `windows-base`이고, 빌드 출력은 `build/preset-windows-base/`입니다.
