...
- 기본 제품의 미사용 CenterPoint 소스는 `legacy/centerpoint_reference/`에 보존했다. 테스트 전용 PCD reader와 초기 설정 제어 코드는 `tests/support/`에 있다. PCD 작업 공간에 남은 이전 소스도 실제 제품 범위는 해당 `WORKSPACE.md`와 CMake 구성을 기준으로 확인한다.
...
기본 버전의 `windows-base`는 이전 v2처럼 **Alazar/FFTW/CUDA ON, CenterPoint OFF**다.
SDK나 CUDA가 없으면 구성 단계에서 실패하므로, 실제 SDK 경로와 CUDA Toolkit 설치를 먼저 확인한다.
장비 없는 CPU 개발은 `windows-base-cpu`를 사용한다. 이 preset은 FFTW만 사용하며 별도의 `build/preset-windows-base-cpu`에 빌드된다.
CPU 실행본을 패키징할 때는 `deploy/windows/package.ps1 -BuildDirectory build/preset-windows-base-cpu/src -OutputDirectory build/package/Windows-Basic-CPU`로 현재 계측 패키지와 구분한다.

PCD 버전은 해당 폴더의 `windows-pcd` preset을 사용한다. PCD의 기본 preset은 여전히 CUDA/Alazar OFF다.
PCD에서 실제 장비 또는 CUDA 처리에는 configure에 `-DFMCW_WITH_ALAZAR=ON -DALAZAR_SDK_ROOT=<SDK root>` 또는 `-DFMCW_WITH_CUDA=ON -DFMCW_REQUIRE_CUDA=ON`을 추가한다. 설치된 기능과 GUI backend를 일치시킨다.
...
- 분리 전 전체 ref bundle, working/staged patch, 변경 파일 사본·SHA256은 기본 폴더의 `build/package_archive/2026-09-22/workspace-separation-backup/`에 있다.
...
과거 실행본은 `build/package_archive/2026-09-22/`, 검증·작업 자료는 `build/package_archive/2026-09-22/workspace-history/`에 있습니다.
예전 CMake cache는 제거했으며 현재 제품 preset의 cache는 유지했습니다. [정리 기록](../archive/file_organization_2026-09-22.md)을 참고하세요.
