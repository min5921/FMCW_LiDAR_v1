...
The uploaded X/Y command order is authoritative only for identifying the fast axis and preserving trajectory provenance. The operator controls line alignment with `scan.bidirectional`: OFF keeps every line in acquisition order, while ON keeps even lines forward and reverses odd lines exactly once into `x_index`. This parity mapping never reverses samples inside an A-scan or changes its FFT input. Cartesian azimuth is derived linearly from the aligned `x_index` and the configured `scan.x_start_deg` to `scan.x_end_deg` range. Vector B-scan lines arrive top-to-bottom, so elevation maps `y_index == 0` to `scan.y_end_deg` and the final line to `scan.y_start_deg`. Source X/Y commands and the sampled trajectory index remain attached as provenance and diagnostics. Generated raster mode uses the same parity and top-to-bottom elevation contract with `records_per_buffer * B_scans_per_frame` points.
...
After a digitizer adapter completes one DMA buffer, it materializes every record descriptor around one pool-backed contiguous `RawFrameBatch`. Simulator and replay batches own their contiguous payload; ATS batches retain a lease on the vendor DMA buffer until the processing or storage consumer releases that input. The session validates and stamps the complete batch before publishing immutable shared ownership to processing and storage. Individual `RawFramePtr` values are aliasing references into that batch, so the pooled allocation is returned only after every consumer releases its final reference. Consumers must not modify sample or metadata memory. UI rendering receives snapshots rather than owning an acquisition buffer.
