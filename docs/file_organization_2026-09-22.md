...
- 문서 목록은 `docs/README.md`, 실행 패키지 안내는 `docs/package_layout.md`입니다.
- 현재 제품 빌드·실행 패키지를 유지하고 과거 패키지는 `build/package_archive/2026-09-22/`에 모았습니다.
- 과거 검증·보고서 생성 스크립트·프로파일링·이미지·로그는 `build/archive/2026-09-22/`에 보관했습니다.
...
`build/organization-20260922/`에 있습니다. 이 폴더와 모든 build 산출물은 Git으로 전달되지 않습니다.
이전 작업 분리 때 만든 `build/workspace-separation-backup-20260922/`도 유지합니다.
