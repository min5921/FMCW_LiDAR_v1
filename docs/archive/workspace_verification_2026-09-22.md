...
> 아래는 작업 공간을 처음 분리했을 때의 기록입니다. 기본 제품은 이후 Alazar/CUDA/FFTW를 포함하도록 복원했습니다. 현재 기본 실행본의 범위와 검증은 [기본 v2 구성](basic_v2_2026-09-22.md)을 확인하세요.
...
이 패키지들의 Alazar adapter는 OFF다. **실제 보드에서 사용할 때에는** 해당 workspace에서 SDK 경로와 `FMCW_WITH_ALAZAR=ON`을 지정해 재빌드·재패키징해야 한다. 기본/PCD에서도 CUDA FFT를 사용하려면 CUDA 옵션을 켜고 재빌드한다. 자세한 명령은 [작업 공간 안내](../guides/workspaces_ko.md)에 있다.
