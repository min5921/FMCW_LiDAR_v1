...
- `docs/archive/phase7_execution_plan.md`의 상태가 구간에 따라 `blocked`와 `in_progress`로 혼재
...
- `docs/hardware/hardware_acceptance.md`의 검증 범위 설명이 이후 실제 ATS/EDFA probe 기록을 반영하지 않음
...
> `docs/archive/review_handoff_2026-08-04.md`를 먼저 읽고 현재 `git status`와 대조해줘. 기존 사용자 변경은 보존하고, 문서에 기록된 P0 문제부터 재현해서 수정 계획 또는 구현을 진행해줘. CTest 9/9는 실시간 hard acceptance 통과가 아니라는 점과 ROS 공식 catkin 빌드가 현재 실패한다는 점을 반드시 반영해줘.
